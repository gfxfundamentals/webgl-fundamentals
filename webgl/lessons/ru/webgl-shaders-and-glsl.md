...
Как говорилось в [как работает WebGL](webgl-how-it-works.html), для WebGL требуется 2 шейдера
...
[Как работает WebGL](webgl-how-it-works.html) посвящён
...
Затем вы получаете ссылку на атрибут в указанной шейдерной программе

    var positionLoc = gl.getAttribLocation(someShaderProgram, "a_position");

и указываете WebGL, как нужно перенести данные из буфера в атрибут
...
В [как работает WebGL](webgl-fundamentals.html) мы рассматривали, что можно
не выполнять какой-либо математики и просто передать данные напрямую.
...
С точки зрения вершинного шейдера uniform-переменные - это значения, постоянные для всех
...
Смотри [Uniform-переменные в вершинном шейдере](#uniforms).
...
Далее получаем ссылку на uniform-переменную в шейдерной программе

    var someSamplerLoc = gl.getUniformLocation(someProgram, "u_texture");

Далее необходимо привязать её к текстурному блоку
...
Если вам интересно узнать о переносе, повороте и масштабе, тогда [начните здесь](webgl-2d-translation.html).
