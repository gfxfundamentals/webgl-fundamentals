...
Итак, мы передали данные в буфер и теперь нужно указать атрибуту, как получить данные
из этого буфера. Для начала необходимо включить атрибут
...
Обратите внимание, что с точки зрения вершинного шейдера GLSL атрибут `a_position` был типа `vec4`
...
[я использую функцию-помощник, о которой вы можете прочесть в](webgl-resizing-the-canvas.html).
...
Наконец, говорим WebGL, какую шейдерную программу нужно выполнить
...
Далее координаты пространства отсечения преобразуются в экранные координаты, в которых
WebGL отрисует треугольник. Если размер canvas будет 400х300, то мы получим следующее:
...
И снова вы могли обратить внимание, что прямоугольник находится внизу. Для WebGL нижний левый угол -
это координаты 0.0. Для того, чтобы начало координат находилось в более привычном верхнем левом углу,
как это принято в 2D-системах, мы можем просто перевернуть y-координату пространства отсечения.
