...
的花哨叫法。对于每一个像素，它会调用你的片元着色器询问你使用什么颜色。
你通过给片元着色器的一个特殊变量`gl_FragColor`设置一个颜色值，实现自定义像素颜色。

使用它们可以做出非常有趣的东西，但如你所见，到目前为止的例子中，
处理每个像素时片元着色器可用信息很少，幸运的是我们可以给它传递更多信息。
想要从顶点着色器传值到片元着色器，我们可以定义“可变量（varyings）”。

一个简单的例子，将顶点着色器计算出的裁剪空间坐标从顶点着色器传递到片元着色器。
...
然后在我们的顶点着色器中定义一个*varying*（可变量）用来给片元着色器传值。
...
可能不值一提的是上例还演示了使用多个属性并且通过顶点着色器向片元着色器传值。
