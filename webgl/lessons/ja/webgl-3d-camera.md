...
この記事はWebGLシリーズのーつである。最初の記事は[WebGLの基本で始まった](webgl-fundamentals.html)。 そして、前回の記事は[透視投影](webgl-3d-perspective.html)についてだった。まだ読んでいなかったら先に読んで下さい。

前回の記事で、`m4.perspective`の関数が原点(0,0,0)から`-zNear`〜`-zFar`の空間を描画するので、「F」をその錘台の中に移動しなければいけなかった。
...
`lookAt`の関数はカメラ以上色々なことで使える。
通常に使う場合はキャラクターの頭を誰かに追いかけることとか。
旋回砲塔を目標に目指すこととか。オブジェクトを通路に追いかけることとか。
そのオブジェクトを通路の何処にあるかを計算して、
ちょっと後通路に何処に存在するかを計算して、その2つの位置を`lookAt`の関数に入れたら、
そのオブジェクトが通路に追いかける行列が出る。その上オブジェクトが通路先に向かう。
...
<h3>lookAtの規格</h3>
<p>
通常に三次元数学ライブラリには<code>lookAt</code>関数がある。それは大抵カメラ行列ではなく、ビュー行列を作成の為に作られた。
