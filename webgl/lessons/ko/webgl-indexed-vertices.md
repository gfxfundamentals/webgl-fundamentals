...
반면에 `gl.drawElements`는 정점 색인으로 채워진 buffer를 사용하고 이를 기반으로 그립니다.
...
하지만, 여전히 삼각형 2개를 그리려면 총 6개의 정점을 그리도록 WebGL에 지시해야 하기 때문에 색인을 사용하는 다른 buffer를 추가해야 합니다.

이를 위해 또 다른 buffer를 생성하지만 다른 바인딩 포인트를 사용하는데요.
...
일반적으로 정점 위치가 사용되는 면에 따라 다른 데이터를 각 정점에 연결하고 싶어 하기 때문에, 색인된 정점은 보통 8개의 정점 위치를 가지는 cube를 만들 수 없다는 점에 유의해야 합니다.
예를 들어 cube의 각 면에 다른 색상을 주고 싶다면 위치와 함께 해당 색상을 제공해야 합니다.
...
<p>
참고: 위에서 색인을 buffer에 넣을 때 <code>indexBuffer</code>를 <code>ELEMENT_ARRAY_BUFFER</code> 바인딩 포인트로 바인딩한 다음 나중에 다시 바인딩합니다.
왜 두 번 바인딩할까요?
</p>
<p>
이건 오로지 패턴을 보여주기 위한 겁니다.
일반적으로 하나 이상의 항목을 그리므로, 그리려는 각 항목에 대해 하나씩, 여러 색인 buffer가 있는데요.
초기화할 때 이러한 buffer를 만들고 데이터를 넣습니다.
렌더링할 때, 각 개별 항목을 그리기 전에 올바른 buffer를 바인딩해야 합니다.
