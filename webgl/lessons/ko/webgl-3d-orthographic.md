...
X와 Y를 픽셀에서 clip space로 변환해줘야 했던 것처럼, Z도 동일한 작업을 해줘야 합니다.
...
참고로 WebGL에서 삼각형이 시계 혹은 반시계 방향으로 진행되는지는 clip space에서 해당 삼각형의 꼭지점에 따라 달라지는데요.
...
한가지 사소한 일이 있는데요.
대부분의 3D 수학 라이브러리에는 clip space에서 픽셀 공간으로 변환을 수행하는 `projection` 함수가 없습니다.
