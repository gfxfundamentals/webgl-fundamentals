...
X와 Y를 pixel에서 clip space으로 변환해야 했던 것처럼, Z도 동일한 작업을 수행해야 합니다.
...
X와 Y를 clip space으로 변환해야 했던 것처럼, Z도 clip space(-1에서 +1)에 있습니다.
...
한 가지 사소한 게 남았는데요.
대부분의 3D 수학 라이브러리에는 clip space에서 픽셀 공간으로 변환을 수행하는 `투영` 함수가 없습니다.
