...
먼저 해야할 일은 shader와 texture를 조정입니다.
...
그리고 전체 텍스처를 'F'의 각 쿼드에 매핑하여 사용하는 좌표를 볼 수 있습니다.
...
반면에 Blender, Maya, 3D Studio Max같은 3D 모델링 소프트웨어에서 3D 모델을 가져온다면, 아티스트(혹은 당신)가 해당 패키지에서 텍스처 좌표를 조정할 겁니다.
...
실제로 2048x2084 픽셀 텍스처가 있고 이를 2x2 픽셀로 그리려 한다고 가정해봅시다.
...
Shader에서 `texture2D`이 호출되고 참조된 텍스처가 제대로 설정되지 않은 경우 WebGL은 검은색(0, 0, 0, 1)을 사용합니다.
...
이 중 하나라도 아니라면 텍스처의 값을 가져올 때 shader에서 검은색(0, 0, 0, 1)을 얻게 됩니다.
...
1) 텍스처 6개를 참조하는 복잡한 shader를 만들고, 사용할 텍스처를 정하기 위해 vertex shader에 fragment shader로 전달되는 추가 정점 정보를 전달합니다.
이건 하지 마세요!
조금만 생각해보면 더 많은 면을 가진 다른 모양에 대해 같은 작업을 수행하려는 경우 수많은 shader를 작성해야 한다는 것을 알 수 있습니다.

2) 큐브 대신 6개의 평면을 그립니다.
이게 일반적인 해결책입니다.
나쁘진 않지만 이 또한 큐브같은 작은 모양에 대해서만 실제로 작동합니다.
1000개의 쿼드로 구성된 구체의 각 쿼드에 다른 텍스처를 적용하려면, 1000개의 평면을 그려야 하고 이는 느립니다.
...
이게 가장 좋은 이유는 로드하는 텍스처가 하나뿐이기 때문에 shader는 하나의 텍스처만을 참조하여 단순하게 유지되며, 평면으로 나누는 경우처럼 텍스처당 한 번씩 그리기 호출을 하는 대신에 단 한번의 그리기 호출만 필요하기 때문입니다.
