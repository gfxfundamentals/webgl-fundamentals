...
이제 머티리얼을 사용해야 합니다.
먼저 parts를 설정할 때 .OBJ 파일에서 가져온 머티리얼의 이름을 사용하고, 로드한 `materials`에서 `material`을 찾는 데 사용할 겁니다.
...
또한 specular map이 없는 머티리얼에 대한 기본값을 추가해야 합니다.
...
.OBJ/.MTL 파일에 관한 제 경험으로, 머티리얼에 대해 항상 약간의 수동 조정이 필요합니다.
어떻게 고칠지는 당신에게 달렸습니다.
...
또한 법선 맵이 없는 머티리얼에 대한 기본 법선 맵을 추가해야 합니다.
...
마찬가지로 머티리얼이 diffuse map을 가지지 않는다면 `uniform sampler2D diffuseMap`이 없는 셰이더를 생성하고 관련한 모든 코드를 제거합니다.
