...
*똑똑한* 대답은 "[Blender](https://blender.org), [Maya](https://www.autodesk.com/products/maya/overview), [3D Studio Max](https://www.autodesk.com/products/3ds-max/overview), [Cinema 4D](https://www.maxon.net/en/products/cinema-4d/overview/)와 같은 3D 모델링 패키지를 사용하세요" 입니다.
...
몇 가지 아이디어가 있습니다.
하나는 원기둥을 만들고 특정 위치에 사인파를 적용하여 올바른 위치에 끼우는 겁니다.
해당 아이디어의 문제는 매끄러운 상판을 얻을 수 없다는 건데요.
표준 원기둥은 동일한 간격을 가진 일련의 고리로 생성되지만 굴곡이 많을수록 더 많은 고리가 필요합니다.

모델링 패키지에서는 2D 실루엣 혹은 2D 실루엣의 가장자리와 일치하는 곡선을 만들어 볼링핀을 만듭니다.
그런 다음 그걸 3D 모양으로 선반 처리하는데요.
*선반*이란 어떤 축을 중심으로 회전하여 점들을 생성한다는 의미입니다.
...
`P3`와 `P5`가 `P4` 양쪽의 정반대에 있을 때 연속된 하나의 곡선처럼 보이는데요.
대부분의 앱은 가파른 곡선을 만들 수 있도록 고정을 푸는 옵션을 함께 제공합니다.
고정 체크 박스를 해제한 다음 `P3`나 `P5`를 드래그해보면 별개의 곡선이라는 것을 훨씬 더 명확하게 알 수 있습니다.

다음으로 곡선의 점들을 생성하는 방법이 필요합니다.
...
그리고 이렇게 곡선에 대한 일련의 점들을 생성할 수 있습니다.
...
그리고 두 끝점으로 이루어진 선에서 가장 먼 점을 찾습니다.
그런 다음 해당 점이 선에서 일정 거리보다 더 멀리 있는지 확인합니다.
그 거리보다 더 가깝다면 두 끝점을 유지하고 나머지를 버립니다.
그렇지 않으면 시작점에서 가장 먼 지점까지 한 번, 그리고 가장 먼 지점에서 끝점까지 한 번 알고리즘을 다시 실행합니다.
...
[간략화한 코드](webgl-less-code-more-fun.html)를 사용하여 다음과 같이 데이터로 WebGL 버퍼를 생성할 수 있습니다.
...
한 가지 문제가 있습니다.
삼각형을 활성화하면 텍스처가 고르게 적용되지 않는 것을 볼 수 있는데요.
이건 `v` 텍스처 좌표가 선에 있는 점의 인덱스를 기반으로 하기 때문입니다.
이들이 균등하게 간격을 둔다면 동작할 수도 있습니다.
하지만 그렇지 않기 때문에 다른 작업을 해야 합니다.

점을 순회하여 곡선의 총 길이와 곡선에 있는 각 점의 길이를 계산할 수 있는데요.
그런 다음 길이로 나누어 `v`에 대해 더 좋은 값을 구할 수 있습니다.
...
구글에 [배럴 UV 매핑](https://www.google.com/search?q=uv+map+a+barrel)이라고 검색해보면 완벽한 UV 좌표를 얻는 것이, 데이터 입력 문제 정도의 수학 문제가 아니며, 해당 데이터를 입력하기 위해 좋은 도구가 필요하다는 것을 알 수 있습니다.
...
법선은 접선에 수직이므로 접선을 사용하면 쉽게 법선을 생성할 수 있을 겁니다
...
한 번 해봅시다.
    ...
    function generateNormals(arrays, maxAngle) {
      ...
      //      3-4
      //     /   \
      //    2     5   S에서 시작하여 E로 돌아가는 원기둥을 내려다보면,
...
`maxAngle`을 더 크게 만들면 인접한 면들이 법선 계산에 포함되기 시작할 때 모서리가 매끄러워지는 것을 볼 수 있습니다.
또한 `divisions`를 5나 6으로 조정한 다음 주변 가장자리가 딱딱하지만 원하는 부분은 부드러워질 때까지 `maxAngle`을 조정해보세요.
...
캡을 잘 다루는 것도 3D 에디터가 도움을 줄 수 있습니다.
선반 처리할 때 제한된 옵션 세트를 사용하는 대신에 에디터의 다른 기능을 사용하여 캡을 추가하고 더 쉽게 캡에 대한 UV를 생성할 수 있습니다.
또한 3D 에디터는 [Extrude Face](https://www.google.com/search?q=extruding+model)와 [Extrude Along Path](https://www.google.com/search?q=extruding+along+a+path)를 지원하는데, 위의 선반 처리 예제를 보면 어떻게 작동하는지 알 수 있습니다.
...
<code>Math.sin(0)</code>과 <code>Math.sin(Math.PI * 2)</code>는 일치해야 하지만 컴퓨터의 부동 소수점 계산은 완벽하지 않으며 충분히 가깝지만 일반적으로 100% 동일하지는 않습니다.
</p>
<p>
이는 법선을 계산하려고 할 때 중요합니다.
우리는 정점이 사용하는 모든 면을 알려고 하는데요.
정점을 비교하여 이를 계산합니다.
만약 2개의 정점이 같다면 같은 정점이라고 가정합니다.
하지만 <code>Math.sin(0)</code>과 <code>Math.sin(Math.PI * 2)</code>가 같지 않기 때문에 동일한 정점으로 간주되지 않습니다.
...
<p>
임의의 3D 포인트를 4x4 행렬로 변환하려면 16번의 곱셈, 12번의 덧셈, 3번의 나눗셈이 필요한데요.
...
<p>
이 최적화에 가치가 있을까요?
글쎄요 이 특정 예제의 경우 그게 중요할 만큼 충분한 작업을 한다고 생각하지 않습니다.
