...
* `POINTS`
   ...
   Vertex shader가 출력하는 각 클립 공간 정점에 대해 해당 점의 중앙에 정사각형을 그립니다.
...
* `LINES`
   ...
   Vertex shader가 출력하는 각 2개의 클립 공간 정점에 대해 두 점을 연결하는 선을 그립니다.
...
* `LINE_STRIP`
   ...
   Vertex shader가 출력하는 각 클립 공간 정점에 대해 vertex shader가 출력한 이전 포인트에서 선을 그립니다.
...
* `TRIANGLES`
   ...
   Vertex shader가 출력하는 3개의 클립 공간 정점마다 점 3개로 삼각형을 그립니다.
   이게 가장 많이 사용되는 모드입니다.
...
* `TRIANGLE_STRIP`
   ...
   Vertex shader가 출력하는 각 클립 공간 정점에 대해 마지막 정점 3개로 삼각형을 그립니다.
...
* `TRIANGLE_FAN`
   ...
   Vertex shader가 출력하는 각 클립 공간 정점에 대해 첫 번째 정점과 마지막 정점 2개로 삼각형을 그립니다.
   다시 말해 6개의 점 A,B,C,D,E,F를 출력하면 삼각형 4개가 그려집니다.
...
특히 법선을 만들거나 texture 좌표를 생성하거나 정점 데이터로 다른 많은 작업을 수행하는 도구가 있을 수 있습니다.
...
추가로, `LINES`는 디버깅이나 간단한 효과에 사용하기는 좋지만 대부분의 플랫폼에서 너비 제한이 1px임을 고려해 볼 때 잘못된 솔루션인 경우가 많습니다.
